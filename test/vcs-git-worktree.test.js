	...
	try {
		...
		await assert.rejects(
			...
			(error) => {
				...
				assert.match(error.message, /stderr: remote: hook rejected token=\[REDACTED\] dbPassword=\[REDACTED\] awsSessionToken=\[REDACTED\]/);
				...
				assert.doesNotMatch(
					error.message,
					/super-secret|db-password-secret|aws-session-secret|db-password-url-secret|aws-session-url-secret|user:secret|access-secret|oauth-secret|credential-secret|client-secret|api-key-secret|user-credential-secret|aws-access-secret|auth-secret|authorization-secret|signature-secret|credential-parameter-secret|security-token-secret|aws-security-token-secret|security-token-camel-secret|legacy-signature-secret|short-signature-secret|basic-header-secret|quoted-bearer-secret|quoted-basic-secret|quoted-token-secret|quoted-api-key-secret|separated-token|separated-api-key|separated-password|separated-token-word|separated-basic|nested-secret/,
				);
...
if (args[0] === "pr" && args[1] === "view") {
  process.stderr.write("GitHub rejected awsaccesskeyid=stderr-access AWSSECRETACCESSKEY=stderr-secret dbPassword=stderr-db-password awsSessionToken=stderr-session note=stderr-context\\n");
  process.stdout.write("response ACCESSKEYID=stdout-access aWsSeCrEtAcCeSsKeY=stdout-secret dbPassword=stdout-db-password awsSessionToken=stdout-session note=stdout-context\\n");
  process.exit(1);
}
`,
	);
	...
	process.env.PATH = `${bin}:${previousPath ?? ""}`;
	const previousUrl =
		"https://github.com/pesap/khala/pull/16?AWSAccessKeyId=url-aws-access&accessKeyId=url-access&AWSSecretAccessKey=url-aws-secret&dbPassword=url-db-password&awsSessionToken=url-session&note=arbitrary-secret";
	try {
		const provider = createGitWorktreeProvider(join(root, "worktrees"), "khala-test/");
		await assert.rejects(
			...
			(error) => {
				...
				assert.match(error.message, /dbPassword=\[REDACTED\]/i);
				assert.match(error.message, /awsSessionToken=\[REDACTED\]/i);
				assert.match(error.message, /note=arbitrary-secret/);
				...
				assert.doesNotMatch(
					error.message,
					/url-aws-access|url-access|url-aws-secret|url-db-password|url-session|stderr-access|stderr-secret|stderr-db-password|stderr-session|stdout-access|stdout-secret|stdout-db-password|stdout-session/,
				);
				assert.ok(error.message.length < 6000);
				return true;
			},
