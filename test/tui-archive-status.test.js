...
test("Work picker stays minimal, shows active Work, and marks failures", async () => {
	...
	assert.match(current, /Scope:.*All/);
	assert.doesNotMatch(current, /home first|ctrl\+h history/);
	assert.match(current, /ctrl\+r refresh/);
	assert.match(current, /r settings/);
	assert.doesNotMatch(current, /ctrl\+h history/);
	assert.match(current, /enter open/);
