...
It does not mutate Khala lifecycle
state or turn review prose into a Verdict.
`/end-review` ends the local review
...
Conclave model.
A `-clarify` marker intercepts the input, places the rewrite in
the editor, and waits for User review.
The extension never silently sends the
