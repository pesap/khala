...
The first view lists Work titles. Each row is a Work, not a Mission. Work is the
User's stable goal; after admission, its Mission is the bounded plan Khala
runs, and its Execution is one attempt under that Mission. The picker hides
...
selector. Work names are bounded before rendering and presented in aligned
...
`khala: idle` or `khala: ◈ 2`. The status view shows `Next`
...
recorded activity. It then provides three focused sections:
...
`?` opens help and `r` opens Role settings. An unreachable Executor exposes
runtime recovery in Actions, and the Conclave can inspect and recover it
without opening the User session. Role settings change the model and thinking level
for future role launches. The help and Role settings keys can be changed in
`khala.json`.
