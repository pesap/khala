const CREDENTIAL_KEY_PATTERN = String.raw`(?:awsaccesskeyid|accesskeyid|awssecretaccesskey|awssecuritytoken|aws[_-](?:access[_-]key[_-]id|secret[_-]access[_-]key|session[_-]token)|securitytoken|password|passwd|(?:[a-z\d]+[_-])*(?:password|passwd)|(?:[a-z\d]+)(?:accesskeyid|secretaccesskey|securitytoken|sessiontoken|password|passwd|token|key|secret|credential(?:s)?|authorization|signature|sig)|credential(?:s)?|auth(?:orization)?|signature|sig|x-amz-(?:signature|credential|security-token)|(?:[a-z\d]+[_-])*(?:token|key|secret|credential(?:s)?)|(?:access|api|auth|client|id|oauth|private|refresh|session)(?:token|key|secret|credential(?:s)?))`;
