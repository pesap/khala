...
description:
  Run an independent read-only review by launching a fresh Pi process with `pi
  -p`, an explicit high-thinking model, and a bounded review packet. Use when
  the user asks for an external review, independent review, second opinion,
  fresh eyes, high-thinking review, or a separate reviewer pass before
  committing, opening, updating, or merging a change.
---

# External Review

Use a separate Pi process as a fresh reviewer. The external reviewer is
advisory: verify its claims locally before changing code or reporting them as
facts.
...
`EXTERNAL_REVIEW_MODEL` when set, falling back to `github-copilot/gpt-5.5`. Keep
`--no-tools` unless the user explicitly wants the external reviewer to use
