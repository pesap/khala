...
description:
  Create a new worktrunk worktree (optionally in another repo) and switch this
  session's working directory into it. Use when launching a session that should
  work in its own worktree (e.g. `/wt-switch-create my-branch -- <task>`, or
  `/wt-switch-create my-branch ~/workspace/other-repo -- <task>`), or
  mid-session to move work into a fresh branch.
argument-hint: "<branch-name> [<repo>] [-- task...]"
license: MIT OR Apache-2.0
compatibility:
  Requires the `wt` CLI (https://worktrunk.dev) and this plugin's WorktreeCreate
  hook
---
...
`../`-relative, or an existing directory) is the repo, and the task starts after
it. Otherwise the task starts at the second token.
...
1. **First action — before reading any files or running any commands:**
   ...
   - It works because this plugin maps `WorktreeCreate` →
     ...
     `.claude/worktrees/`.
   - `wt switch --create` is idempotent: if the branch already exists, this just
     re-enters its worktree.
   - If you are _already_ inside an `EnterWorktree`-created worktree (e.g. the
     ...
     was given and that worktree belongs to a different repo, you can't honor it
     — say so and stop rather than running the task in the wrong repo.
