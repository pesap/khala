...
agent will work from. The original body, discussion, and attached diff are
context — the agent brief is the contract.

The brief states what the agent should do across both surfaces: for an issue,
that is building the change from nothing; for a PR, that is what remains to do
to the existing diff — finish it, close gaps, or address review points.
...
produces descriptions that end in the middle of a word.
...
- Descriptions over 1024 chars are truncated at the last word boundary before
  1024 chars
...
links to all issues that requested the feature. When triage reviews new issues,
these files should be checked for matches.
...
- During triage, existing `.out-of-scope/` files are checked and surfaced when a
  new issue matches a prior rejection
...
structure. Two gaps remain: errors are still printed as human text, and the new
flag has no test coverage.
