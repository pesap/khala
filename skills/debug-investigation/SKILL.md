...
description:
  Diagnose bugs from first principles by reproducing signals, ranking
  hypotheses, validating evidence, and converging on a durable root cause before
  proposing or applying fixes. Use this skill when users ask to debug failures,
  investigate flaky behavior, explain why something broke, or understand a
  regression without guess-and-patch changes.
license: MIT
---
...
6. Propose durable fix direction.
   ...
   - When invoked by `/debug`, stop at an issue-ready brief instead of applying
     code changes.
...
- Jumping to a likely fix before reproducing or locating the earliest
  trustworthy signal.
