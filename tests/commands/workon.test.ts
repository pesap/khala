...
test("groups multiple GitHub issues into one capsule and Worktrunk session", async () => {
  ...
  try {
    ...
    assert.match(capsule, /Implementation instructions:/);
    assert.doesNotMatch(capsule, /Implementation instructions for an explicit follow-up only:/);
...
test("waits for Worktrunk Zellij tab before launching Pi pane", async () => {
  ...
  try {
    ...
    assert.match(scriptCall, /--prompt ## Worker continuation context/);
    assert.doesNotMatch(scriptCall, /## Deterministic \/workon route/);
    assert.match(scriptCall, /Parent \/workon route: launched/);
    assert.match(scriptCall, /The parent\/operator stop contract does not apply in the worker pane\./);
    ...
    assert.doesNotMatch(scriptCall, /Allowed action: report the deterministic launch state and stop\./);
    assert.doesNotMatch(scriptCall, /Final response: report readiness, source issue, branch\/worktree, capsule path, Pi pane, heartbeat pane, next operator action, risks, result, and confidence\./);
    assert.doesNotMatch(scriptCall, /do not implement, review, run Reviewer Two, create PRs, or continue worker tasks from the operator\/bootstrap surface/);
    assert.doesNotMatch(scriptCall, /do not relaunch or create alternate tabs; do not merge, mark ready, close issues\/PRs, label, or post broad public comments unless explicitly told/);
