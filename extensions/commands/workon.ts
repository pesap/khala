...
const DEFAULT_HANDOFF_TEMPLATE = `I want to discuss and possibly work on: {{issue_title}}
...
async function readHandoffTemplate(cwd: string): Promise<string> {
  const templatePath = path.join(cwd, "commands", "workon-handoff-template.md");
  try {
...
function heartbeatLabel(value: string): string {
  return `${value} (HOURS.MINUTES; e.g. 0.15 = 15 minutes, 2.0 = 2 hours)`;
}
