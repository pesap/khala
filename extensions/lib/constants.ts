...
export const MUTATION_BASH_PATTERN = /(?:^|\n|[;|&]{1,2})\s*(?:git\s+(?:add|apply|am|commit|checkout|switch|merge|rebase|cherry-pick|revert|reset|restore|clean|stash|tag|branch|push|pull)(?=\s|$|[;|&])|(?:npm|pnpm|yarn|bun)\s+(?:install|add|remove|update|upgrade|ci)(?=\s|$|[;|&])|(?:pip|pip3|uv)\s+(?:install|sync|add|remove)(?=\s|$|[;|&])|(?:node|bun|deno)\b[^\n;|&]*(?:writeFile(?:Sync)?|appendFile(?:Sync)?|rm(?:Sync)?|unlink(?:Sync)?|mkdir(?:Sync)?)|(?:python|python3|uv\s+run\s+python)\b[^\n;|&]*(?:open\([^)]*["'](?:w|a|x)\b|write_text\(|write_bytes\(|unlink\(|mkdir\()|ruby\b[^\n;|&]*(?:File\.(?:write|open|delete|unlink)|Dir\.mkdir)|[^;\n|&]*(?:[12]?>|>>|&>)|rm\b|mv\b|cp\b|mkdir\b|rmdir\b|touch\b|chmod\b|chown\b|sed\b[^\n;|&]*\s-i\b|perl\b[^\n;|&]*\s-i\b|tee\b|truncate\b|dd\b)/m;
