...
export async function getBootstrapPayload(params: {
  ...
  return [
    ...
    "- For mutation or memory-write turns, including mutating shell commands such as `sed -i`, package-manager edits such as `npm install`, VCS writes such as `git commit` or `but commit`, and file-producing commands such as `dd of=README.md`, the latest focused khala_search_memory before the first edit/write/mutating bash/khala_learn must succeed and still be fresh; searching memory after mutation, relying on a stale search after several non-memory tools, or relying on an older success after a later failed search is too late to satisfy task-specific recall.",
    ...
    "- Avoid unbounded local evidence dumps such as `cat file`, `nl -ba file`, `bat file`, `less file`, `more file`, scripted dumps like `awk '{print}' file` or `python -c \"print(open('file').read())\"`, bare `rg --files`, command-substitution fanouts like `sed -n '1,80p' $(rg --files)`, bare repo-wide `rg TODO`, raw `git diff`/`git show`/`but diff` patch output, unbounded `git log`/`git reflog` history output, raw `git status`/`git branch` state checks in GitButler workspaces, broad repo summaries like `tree .` or `du -ah .`, VCS commands that combine summary flags with `-p`/`--patch`, watch/follow commands like `--watch` or `tail -f`, broad `sed -n` print ranges, excessive `head`/`tail` or `grep`/`rg` match limits, `grep -R`, broad `find .` forms like `find . -maxdepth 2 -type f`, or `ls -R`; use bounded read/search tools, summary flags, one-shot test flags, GitButler summaries like `but status -fv`, focused `rg --files -g` globs or paths, shallow tree limits like `tree -L 2`, focused find predicates like `find . -maxdepth 3 -name package.json`, explicit history limits like `git log -5`, `rg` with small limits, scoped paths, or add small explicit limits when shell is necessary.",
    ...
    "- For obvious best-practice task classes, read the relevant packaged SKILL.md before finalizing even if the user did not name it: code review, debugging, TDD, skill creation, security audit, docs authoring, GitHub work, commit/GitButler work, academic/first-principles review, dependency untangling, data modeling, API design, dead-code proof, type hardening, public API compatibility, feature delivery, Python, pytest, uv, Rust, infrasys, Bash scripts, CLI UX, TypeScript, and simplification.",
