...
export function buildLearnedWorkflowArtifact(params: {
  ...
  return [
    ...
    "steps:",
    "  - read_memory: call khala_read_memory before non-memory mutations",
    ...
    "  - validate: run targeted validation or record the exact blocker",
    "  - eval: define reusable validation prompts or checks for this workflow pattern",
    "  - learn: store or patch durable non-sensitive lessons",
    "artifactRequirements:",
    "  triggers: explicit use-when conditions and near-miss boundaries",
    "  inputs: expected user input, repo state, and required evidence",
    "  outputs: expected final artifact or response shape",
    "  validation: commands, checks, or eval prompts that prove the workflow still works",
    "  reuse: instructions for when to rerun, update, or retire this workflow",
    "promotionEvidence:",
...
export function buildLearnedWorkflowPromptTemplate(params: {
  ...
  return [
    ...
    "- Call khala_read_memory before non-memory mutations.",
    ...
    "- Validate the result with targeted commands, checks, or eval prompts; explain the exact blocker if validation is impossible.",
    "- When the run creates or updates a reusable workflow, include triggers, inputs, outputs, ordered steps, validation/eval prompts, and reuse instructions.",
